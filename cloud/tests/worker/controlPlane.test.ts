...
describe('Discord OAuth and Activity session', () => {
  it('returns bounded failures when Discord rejects the exchange', async () => {
    ...
    await expect(oauthToken(jsonRequest('/api/oauth/token', { code: 'bad-code' }), env as unknown as Env)).rejects.toMatchObject({
      status: 401,
      code: 'oauth_failed',
  ...
  it('establishes identity server-side and sets the iframe-safe cookie', async () => {
    ...
    const response = await oauthToken(jsonRequest('/api/oauth/token', { code: 'real-code' }), env as unknown as Env)
...
describe('pairing', () => {
  it('retries a hash collision and consumes a code only once', async () => {
    ...
    await env.DB.prepare(
      ...
      .bind(await pairingHash('ABCDEFGH', workerEnv), ACCOUNT_ID, Date.now() + 60_000, Date.now())
      .run()
    const codes = ['ABCDEFGH', 'BCDEFGHJ']
    const result = await createPairing(ACCOUNT_ID, request, workerEnv, { codeSource: () => codes.shift()! })
    expect(result.code).toBe('BCDEFGHJ')
    expect(await consumePairing(result.code, workerEnv)).toBe(ACCOUNT_ID)
    await expect(consumePairing(result.code, workerEnv)).rejects.toMatchObject({ code: 'invalid_pairing_code' })
  })
  ...
  it('rejects expired codes and bounds account attempts', async () => {
    ...
    const expired = await createPairing(ACCOUNT_ID, request, workerEnv, { codeSource: () => 'CDEFGHJK', now: 1000 })
    await expect(consumePairing(expired.code, workerEnv, 1000 + 5 * 60 * 1000)).rejects.toMatchObject({
      code: 'invalid_pairing_code'
    })
    for (let index = 0; index < 5; index += 1) {
      await createPairing(ACCOUNT_ID, request, workerEnv, { codeSource: () => `DEFGHJK${index + 2}` })
    }
    await expect(createPairing(ACCOUNT_ID, request, workerEnv, { codeSource: () => 'EFGHJKLM' })).rejects.toMatchObject({
      code: 'rate_limited'
    })
  })
})
...
describe('devices and tickets', () => {
  it('hashes the issued secret, pairs once, and prevents sessions after revocation', async () => {
    ...
    const pairing = await SELF.fetch(jsonRequest('/api/pairing', {}, cookie)).then((response) => response.json()) as {
      code: string
    }
    const pairResponse = await SELF.fetch(jsonRequest('/api/devices/pair', { code: pairing.code, label: 'Laptop' }))
    const device = (await pairResponse.json()) as { deviceId: string; deviceSecret: string; discordName: string }
    ...
    expect((await SELF.fetch(jsonRequest('/api/devices/pair', { code: pairing.code }))).status).toBe(400)
    expect((await SELF.fetch(jsonRequest('/api/devices/session', device))).status).toBe(200)
  ...
  it('issues role-scoped, expiring, single-use tickets', async () => {
    ...
    const publisher = (await SELF.fetch(jsonRequest('/api/devices/session', device)).then((response) => response.json())) as {
      ticket: string
    }
    const viewer = (await SELF.fetch(jsonRequest('/api/viewer/session', {}, cookie)).then((response) => response.json())) as {
      ticket: string
    }
    const first = await SELF.fetch(`https://worker.test/api/sync?ticket=${publisher.ticket}`, { headers: { upgrade: 'websocket' } })
    expect(first.status).toBe(101)
    expect((await SELF.fetch(`https://worker.test/api/sync?ticket=${publisher.ticket}`, { headers: { upgrade: 'websocket' } })).status).toBe(401)
    ...
    expect((await SELF.fetch(`https://worker.test/api/sync?ticket=${viewer.ticket}`, { headers: { upgrade: 'websocket' } })).status).toBe(401)
  ...
  it('reports account/device status and enforces revoke ownership', async () => {
    ...
    const cookie = await sessionCookie()
    const device = await seedDevice()
    const me = await SELF.fetch(new Request('https://worker.test/api/me', { headers: { cookie } }))
    expect(await me.json()).toMatchObject({ paired: true, devices: [{ id: device.deviceId, label: 'Test desktop' }] })
    expect(
      (await SELF.fetch(new Request('https://worker.test/api/devices/not-owned', { method: 'DELETE', headers: { cookie } }))).status
    ).toBe(404)
  })
})
